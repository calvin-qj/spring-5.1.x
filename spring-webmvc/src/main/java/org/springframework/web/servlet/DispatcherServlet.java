...
public class DispatcherServlet extends FrameworkServlet {
	...
	/**
	 * Initialize the strategy objects that this servlet uses. 初始化Servlet是要使用的策略
	 * <p>May be overridden in subclasses in order to initialize further strategy objects.
	 * 它的作用是通过反射机制查找并装配Spring容器中用户显示自定义的组件Bean，
	 * 如果没有显示自定义的组件Bean，则装配默认的组件实例。这个方法被调用的时机是在容器启动的时候，
	 */
	...
	protected void doService(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// Keep a snapshot of the rezquest attributes in case of an include,
		...
		// Make framework objects available to handlers and view objects. 设置Spring上下文
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				//去掉处理当前请求的Controller 这里也称为Handler 即处理器
				//这里不是直接返回Controller 而是返回HandlerExecutionChain请求处理器链对象 该对象封装了Handler和interceptor
				...
				//HttpRequestHandlerAdapter是用来适配远程调用的。SimpleServletHandlerAdapter是用来适配Servlet实现类的
	 ...
	 * 从 HandlerMapping 中找到 URL和 Controller 的对应关
	 * 系 ，也就是 Map<url,Controller＞。 我们知道，最终处理请求的是 Controller 中的方法，现在只是知
	 * 道了 Controller
	 */
	@Nullable
	...
	/**
	 * Return the HandlerAdapter for this handler object. 该处理程序对象的处理程序适配器
